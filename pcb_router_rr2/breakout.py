...
With use_breakout=False the agent grows directly from the pins and this module
is never invoked. Retained for boards where a fixed escape fan is preferred.
"""
...
def _group(cfg, board, idxs, sign, allow_jogs):
    pins = board.pins
    cx0, cy0, cx1, cy1 = board.connector_rect
    conn_cx = 0.5 * (cx0 + cx1)
    clear_y = (cy1 if sign > 0 else cy0) + sign * (
        cfg.obstacle_clearance_mm + cfg.breakout_clear_margin_mm)
    ...
    if allow_jogs:
        for i in idxs:
            ...
            if any(j != i and abs(pins[j, 0] - xi) < cfg.trace_clearance_mm
                ...
                lane_x[i] = xi - cfg.breakout_lane_jog_mm if xi <= conn_cx \
                    else xi + cfg.breakout_lane_jog_mm
    ...
    pos = {i: lane_x[i] for i in idxs}; y = clear_y
    ...
    while max(abs(pos[i] - fan_x[i]) for i in idxs) > 1e-6:
        ...
        if n > 1:
            mg = float(np.min(np.diff(cur)))
            if mg <= clr_safe:
                raise ValueError(f"Fan gap collapsed to {mg:.3f} mm")
            rate = math.sqrt((mg / clr_safe) ** 2 - 1.0)
        else:
            rate = math.inf
        max_dx = rate * drop; y += sign * drop
        for i in idxs:
    ...
    for i in idxs:
        ...
        pts.append([lane_x[i], clear_y]); pts.extend(fan_pts[i])
...
def build_breakout(cfg: Config, board: Board) -> List[np.ndarray]:
    ...
    if mode == "split":
        ...
        raw = {}
        raw.update(_group(cfg, board, top, +1, False))
        raw.update(_group(cfg, board, bot, -1, False))
    else:
        raw = _group(cfg, board, list(range(n)), +1 if mode == "up" else -1, True)
    polys = [np.asarray(raw[i], dtype=np.float64) for i in range(n)]
    lengths = [_len(p) for p in polys]; target = max(lengths)
    for i in range(n):
        d = target - lengths[i]
        if d > 1e-9:
            v = polys[i][-1] - polys[i][-2]
            v = v / (np.linalg.norm(v) + 1e-12)
            polys[i] = np.vstack([polys[i], polys[i][-1] + v * d])
    polys = [_simplify(p) for p in polys]
    lengths = [_len(p) for p in polys]
    assert max(lengths) - min(lengths) < 1e-6, "breakout normalisation failed"
    _validate(cfg, board, polys)
    return polys


def _validate(cfg: Config, board: Board, polys: List[np.ndarray]) -> None:
    ...
    for i, pts in enumerate(polys):
        for k in range(len(pts) - 1):
            ...
            if k > 0 and not (checker.point_in_board(a) and checker.point_in_board(b)):
                raise ValueError(f"Breakout trace {i} seg {k} leaves board")
            for r in inner:
                if seg_intersects_rect(a, b, r):
                    raise ValueError(f"Breakout trace {i} seg {k} crosses a keep-out")
            ...
            for a0, a1 in zip(polys[i][:-1], polys[i][1:]):
                for b0, b1 in zip(polys[j][:-1], polys[j][1:]):
                    ...
                    if d < cfg.trace_clearance_mm - 1e-6:
                        raise ValueError(f"Breakout {i}/{j} clearance {d:.3f} mm")
